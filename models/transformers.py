...
def hyperopt_2towers(hp):
    ...
    embedding_spac = Dense(units=embedding_units_spac, activation=activation_function,
                           kernel_initializer=initializer_map[activation_function], kernel_regularizer='l1')(inp)
    embedding_temp = Dense(units=embedding_units_temp, activation=activation_function,
                           kernel_initializer=initializer_map[activation_function], kernel_regularizer='l1')(
        inp_transposed)
    ...
    dense = Dense(units=last_layer_units, activation=activation_function,
                  kernel_initializer=initializer_map[activation_function], kernel_regularizer='l1')(concat)
    ...
    model.compile(loss=softmax_cross_entropy, optimizer=Adam(learning_rate=start_lr),
                  metrics=['accuracy', F1Score(num_classes=3, average='macro')])
...
def create_stacked_encoder(x_train, n_classes):
    ...
    embed = Dense(units=6, activation=tf.keras.layers.LeakyReLU(alpha=0.3), kernel_initializer=HeNormal(),
                  kernel_regularizer='l1_l2')(norm)

    enc = SinePositionEncoding()(embed)

    added = Add()([enc, embed])

    stack = TransformerEncoder(intermediate_dim=10, num_heads=2, dropout=0.1, activation=tf.keras.layers.LeakyReLU(alpha=0.3),
    ...
    output = Dense(units=n_classes)(dense)
